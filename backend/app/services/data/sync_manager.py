...
class DataSyncManager:
    ...
    async def sync_minute_kline(
        ...
        if not start_date:
            # 查找最新数据
            last = (
                self.db.query(StockMinuteKline.trade_time)
                .filter(
                    StockMinuteKline.symbol == symbol,
                    StockMinuteKline.period == period,
                )
                .order_by(StockMinuteKline.trade_time.desc())
                .first()
            )
            if last:
                start_date = last[0].date()
            else:
                start_date = date.today() - timedelta(days=5)  # 默认最近5天
        ...
        try:
            ...
            logger.error(f"同步分钟线 {symbol} {period} 失败: {e}")
            return {"symbol": symbol, "inserted": 0, "error": str(e)}
        ...
        existing_times = set(
            r[0]
            for r in self.db.query(StockMinuteKline.trade_time)
            .filter(
                StockMinuteKline.symbol == symbol,
                StockMinuteKline.period == period,
                StockMinuteKline.trade_time >= datetime.combine(start_date, datetime.min.time()),
                StockMinuteKline.trade_time <= datetime.combine(end_date, datetime.max.time()),
            )
            .all()
        )
        ...
        new_rows = []
        for _, row in df.iterrows():
            trade_time = row["trade_date"]  # 已经是 datetime
            if trade_time not in existing_times:
                new_rows.append({
                    "symbol": row["symbol"],
        ...
        if new_rows:
            ...
            for i in range(0, len(new_rows), batch_size):
                batch = new_rows[i: i + batch_size]
                self.db.bulk_insert_mappings(StockMinuteKline, batch)
            self.db.commit()

        logger.info(f"同步分钟线 {symbol} {period} 完成: 插入 {len(new_rows)} 条")
        return {"symbol": symbol, "period": period, "inserted": len(new_rows)}
