...
class StrategyExecutor:
    ...
    def validate_code(code: str) -> tuple[bool, str]:
        ...
        try:
            ...
            if "generate_signals" not in namespace:
                return False, "缺少 generate_signals(df, params) 函数"
            return True, "通过"
        except SyntaxError as e:
            return False, f"语法错误: {e}"
        except Exception as e:
            return False, f"代码错误: {e}"
    ...
    def generate_signals(
        ...
        try:
            ...
            if "generate_signals" not in namespace:
                return {"status": "failed", "error": "缺少 generate_signals(df, params) 函数"}

            generate_signals = namespace["generate_signals"]
            result = generate_signals(kline_data.copy(), params)

            if not isinstance(result, pd.DataFrame) or "signal" not in result.columns:
                return {"status": "failed", "error": "必须返回包含 'signal' 列的 DataFrame"}
